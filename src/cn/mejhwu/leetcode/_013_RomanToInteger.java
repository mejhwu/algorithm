...
public class _013_RomanToInteger {
    ...
    public int charMapToInteger(char ch) {
        switch (ch) {
            case 'M':
                return 1000;
            case 'D':
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < s.length() - 1; i++) {
            int top = charMapToInteger(s.charAt(i));
            int next = charMapToInteger(s.charAt(i + 1));
            if (top < next) {
                num -= top;
            } else {
                num += top;
        ...
        num += charMapToInteger(s.charAt(s.length() - 1));
        return num;
    }

    @Test
    public void testRomanToInt() {
        _013_RomanToInteger o = new _013_RomanToInteger();
        assertEquals(3, o.romanToInt("III"));
        assertEquals(4, o.romanToInt("IV"));
        assertEquals(9, o.romanToInt("IX"));
        assertEquals(58, o.romanToInt("LVIII"));
        assertEquals(1994, o.romanToInt("MCMXCIV"));
        assertEquals(1994, o.romanToInt("MCMXCIV"));
    }
